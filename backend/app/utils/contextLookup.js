...
function buildLiteraryDictionaryPrompt({ word, workTitle }) {
  return `
...
function parseLookupModelResponse(responseText) {
...
async function lookupWithGemini({ word, workTitle, apiKey }) {
  ...
  const result = normalizeGeminiEntry(
    parseLookupModelResponse(responseText),
    word,
  );
...
async function lookupWithOllama({ word, workTitle }) {
  ...
  const response = await fetchWithTimeout(`${OLLAMA_HOST}/api/generate`, {
    ...
    body: JSON.stringify({
      model: OLLAMA_LOOKUP_MODEL,
      prompt: buildLiteraryDictionaryPrompt({ word, workTitle }),
      format: "json",
      stream: false,
    }),
  ...
  if (!response.ok) {
    throw new LookupError("Ollama lookup failed.", 502);
  }

  const data = await response.json();
  const result = normalizeGeminiEntry(
    parseLookupModelResponse(data?.response),
    word,
  );
...
async function lookupContext({ word, workTitle, provider = "gemini", apiKey }) {
  ...
  if (provider === "gemini") {
    return lookupWithGemini({ word, workTitle, apiKey });
  }

  throw new LookupError("Unsupported lookup provider.", 400);
}
