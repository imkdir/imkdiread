...
function createApp(options = {}) {
  ...
  const dbPath = options.dbPath || process.env.DB_PATH || path.join(__dirname, "db", "database.sqlite");
...
const parseWorkPayload = (rawWork) => {
  ...
  const authors = rawWork.authors === undefined ? [] : asStringArray(rawWork.authors);
  ...
  if (
    (rawWork.goodreads_id !== undefined && rawWork.goodreads_id !== null && typeof rawWork.goodreads_id !== "string") ||
    (rawWork.dropbox_link !== undefined && rawWork.dropbox_link !== null && typeof rawWork.dropbox_link !== "string") ||
    (rawWork.amazon_asin !== undefined && rawWork.amazon_asin !== null && typeof rawWork.amazon_asin !== "string")
  ) {
...
app.post("/api/works/bulk-import", authenticateToken, requireAdmin, (req, res) => {
  try {
...
app.post("/api/works/bulk-tags", authenticateToken, requireAdmin, (req, res) => {
  try {
    const { workIds, tags } = req.body;
    const normalizedWorkIds = asStringArray(workIds);
    const normalizedTags = asStringArray(tags);
    if (!normalizedWorkIds || !normalizedTags || !normalizedWorkIds.length || !normalizedTags.length) {
...
app.put("/api/quotes/:id", authenticateToken, (req, res) => {
  try {
    ...
    const pageNumber = pageNumberRaw === null || pageNumberRaw === undefined || `${pageNumberRaw}`.trim() === ""
      ? null
      : normalizePageNumber(pageNumberRaw);
    const userId = req.user.id;
    const quoteId = req.params.id;
    if (!quote) return jsonError(res, 400, "Quote text is required.");
    if (pageNumberRaw !== undefined && pageNumberRaw !== null && pageNumber === null) {
      return jsonError(res, 400, "pageNumber must be a positive integer.");
    }
